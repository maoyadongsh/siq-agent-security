...
用户随后**全局取消 CodeBuddy 的后续任务与适配**。CodeBuddy 不属于上述任一 OS 的当前交付矩阵；历史文档、旧诊断、旧配置、Grant、回执和 hook 的存在不构成当前支持声明。新安装、新 Grant 和历史待激活 Grant 的启用入口须拒绝；历史配置允许只读查看、安全卸载，旧 Grant 允许读取、拒绝或撤销，旧 hook 在退出前保持失败关闭。后续任务书不得给 CodeBuddy 新开发、宿主验收或发布承诺。WorkBuddy 与 CodeBuddy 是不同宿主，不得互相代替证据。
...
本机 F08 从此只继续托管源的真实网络预检与安全安装路径；Linux/WorkBuddy 诊断结果仅用于说明历史环境事实，不再作为待解锁的本机开发依赖。不得为“消除阻塞”借用 CodeBuddy、模拟宿主、放宽权限/SSRF 或把范围排除写成 `passed`。若未来恢复 Linux/WorkBuddy 或 CodeBuddy，需新的产品决定、真实宿主与版本、适配方案及完整原生验收，不能沿用此次范围排除。
