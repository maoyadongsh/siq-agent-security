...
- 当前状态（2026-09-07 22:36）：当前适配器与内嵌安装资产已集成宿主能力识别和审批后重查；缺能力宿主在 block 下拒绝 hold，配套宿主保留正常审批功能。18 个原生集成场景及本地 Go race/vet、四目标构建通过；宿主升级/回退工具及 15 项中断恢复回归已落盘，完整副本升级和回退后原生验证通过；真实安装未更新，本轮代码尚未提交。`87fd1ce` 的 CI 28/28 成功属于此前基线。完整旧版本兼容、真人审批、消息渠道、CodeBuddy 实机与独立复核仍待验收。
