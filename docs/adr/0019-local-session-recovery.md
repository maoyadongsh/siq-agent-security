...
2026-09-25 补充：按用户明确选择，新源码的管理会话改为固定 24 小时，响应使用 `local-admin-session/v2`，旧 v1 合同与历史证据保留。另以 `local-browser-connect/v1` 增加用户发起、Skill 本机确认、原浏览器 Cookie 单次领取的便捷连接；不改变权限审批、同 UID 边界、重启失效或恢复不续期原则。实现约束与负向条件见[开发规格 §3.11.1](../agentshield-dev-spec-v1.md#3111-个人客户端初始化2026-09-12ux-003)。下文 12 小时为原决策记录，不是新版本有效期。
