...
- 日期：2026-09-08
- 状态：采纳；运行时、受控observer与最小Completion已有组件证据，平台支持边界另行验证
...
运行时正负测试已覆盖普通 token 伪造独立来源、未知 action、回执错配、假成功、拒绝后实际效果、证据冲突、重复提交和重启恢复。合同测试只证明结构及自报上限，不作为这些运行时门禁已完成的证据。

同一动作的独立完成与失败材料并存时，Completion保留双方并返回conflicting；单独失败仍incomplete，不同动作不因此自动合并成矛盾。该规则不推断文件后来被删除的业务原因，也不将工具自报升级为独立证明。
