...
| --- | --- | --- |
| A fixture UI | Linux aarch64 on the existing DGX host, explicit test provider; all nine browser scenarios passed | CPU/test path, no model call or GPU requirement; independent ordinary x86 Linux not yet run |
...
The same physical DGX host executing CPU fixtures does not establish independent non-DGX reproduction. CI on hosted Ubuntu supplies additional Linux evidence when the research PR completes; it does not retroactively make the local UI test an x86 result. Four-platform cross compilation is not native testing of all platforms.
