...
核查日期：2026-09-19。产品与发布基线为 `main@1173042`；整理工具首批为 `ea0024f`。开始工作前以 `git fetch origin main`、`git status --short --branch` 和 `git rev-parse HEAD origin/main` 确认自己检出的身份；本页不是移动分支指针。正式客户端 `0.3.0` 的源码固定 `83fde2d`，研究源码版仍为 `aefab111`。
...
| --- | --- | --- | --- |
| 仓库组织 / 主线维护者 | [调整方案](../repository-architecture-reorganization-proposal-20260918-095230.md)、[RA 进度](reorganization-progress.md) | 用户已授权持续实施；冻结/路径守卫已建立 | 按 RA 批次补导航、索引和发行工具；不改变运行时安全语义 |
...
| 客户端发行 / 发行维护者 | [打包与安装](../signed-release-packaging.md)、[0.3.0 记录](../evidence/releases/0.3.0/README.md) | 普通 Release、Latest；14/14 包检查、8/8 回读、5/5 源码工作流分别记账 | 沉淀通用最终包验证/回读；新版本原生验收继续，不重签或覆盖 0.3.0 |
...
- [Windows 分支索引](../windows-branch-pr-status-20260918.md)的待合并栈与整合报告的待签状态属于历史；以本页链接的源码/发行边界后续记录判定替代范围。
