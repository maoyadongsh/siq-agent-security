...
日期：2026-09-18。状态：用户级/项目级生命周期组件与管理 HTTP 接线已验证，管理界面已实现并构建；原生加载与完整旅程仍未验收。初始只读核对基线为 `edbf5d1e5a52a92f126f58302a59fd4dc27f60d2`；后续合同、安装库、服务端和界面按本文接线，开发检查与真实宿主结果分别记录。资源编辑、安装引导和通知改动另有专项规格。
...
先冻结合同字段和样例，再分工；同一文件单一写入负责人。以下为实现文件的职责边界；实际修改与检查见当前交付记录。
...
下列是完整完成门槛；已运行部分以当前交付记录为准，固定候选与失败证据分别保留。
