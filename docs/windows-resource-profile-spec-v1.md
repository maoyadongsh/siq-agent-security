...
运行时接通细节见 [Windows Authority 接通增量](windows-authority-runtime-v1.md)。
...
状态写入门禁覆盖 GrantCommit、PutGrant/CAS 和底层版本化写入。判断 grants 命名空间时必须先按当前 OS 清理分隔符与导航分量，并在 Windows 上按不区分大小写比较，不能让 `GRANTS`、`grants/.` 等同目录拼写绕过检查。拒绝发生在目录、暂存文件或事务日志创建前；不替代现有状态目录路径检查。
...
Windows 固定导入 Skill 的权限转换按 [WorkBuddy Skill 安装增量](windows-workbuddy-skill-install-v1.md) §4.1 接通既有 grant/v2 SkillRef 字段：保留完整来源与 Skill 身份，只接受受验证的保留导入准入和受管实例主体，不能将普通 Skill 授权或缺失归属的导入授权转换为实例基线。准备修订仍需明确确认，来源快照复验由管理入口执行，历史验签/撤销保持离线可用。
