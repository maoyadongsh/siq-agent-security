...
root=base/'.tmp/win-task-native/bootstrap-fixed-20260917/skill-native-private-r2';root.mkdir(exist_ok=True);home=root/'home';home.mkdir(exist_ok=True);state=root/'state'
fixture=root/'read-acl.ps1'
...
host=home/'.openclaw';host.mkdir(exist_ok=True);config=host/'openclaw.json';original_config=b'{"gateway":{"mode":"local"},"fixture":"retain"}\n';assert config.read_bytes()==original_config
...
def save(): (root/'resume-result.private.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
...
def api(label,method,path,body=None,want=200,**kw):
 ...
 return out
report['resume_of']='result.private.json';report['resume_reason']='fixture inspected runtime_verified at wrong level after successful install; continue saved installation without reinstall'
try:
 port=json.loads((root/'01-init.stdout.private').read_text())['port']
 with socket.socket() as sock:sock.settimeout(1);assert sock.connect_ex(('127.0.0.1',port))!=0,'port reused'
 log=(root/'resume-daemon.private.log').open('wb')
 ...
 check('owned-daemon-restarted',True)
 code=api('16-pairing','POST','/v1/session/pairing',credential=(state/'admin-recovery.token').read_text().strip(),headers={'X-SIQ-Local-CLI':'1'})['code']
 session=api('17-pair','POST','/v1/pair',{'code':code},credential='')['session']
 actor='acl-fixture-human';saved=json.loads((root/'12-install.response.private.json').read_text(encoding='utf-8'))
 installroute='/v1/skill-installations/operations/'+saved['install_id']
 installed=api('18-read-installed','GET',installroute)
 check('installed-unverified-readback',installed['status']=='installed_unverified' and installed['operation']['runtime_verified'] is False and installed['plan']['runtime_verified'] is False)
 target=host/'skills/acl-fixture/SKILL.md';check('actual-content-copied',target.read_bytes()==(root/'source/SKILL.md').read_bytes())
 ...
 view=api('19-removal-preview','GET',installroute+'/removal')
 removed=api('20-remove','POST',installroute+'/removal',{'schema_version':'local-skill-install-remove/v1','operation_signature':installed['operation']['signature'],'expected_grant_revision':view['state_revision'],'expected_binding_signature':view['binding_signature'],'actor_id':actor,'confirm_remove':True})
 check('owned-target-removed',not target.exists());check('original-config-preserved',config.read_bytes()==original_config)
 check('signed-stop',cli('21-stop','stop','--confirm-stop')==0);check('owned-daemon-exit',child.wait(timeout=15)==0)
 report['passed']=True
finally:
