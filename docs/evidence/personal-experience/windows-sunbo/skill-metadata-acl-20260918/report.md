...
测试未启动真实 OpenClaw/Hermes 进程、未调用模型，不能替代真实宿主加载或工具保护证据。更新计划/替换/旧授权撤销现已有原生 SIQ API 证据；调度配置仍为组件测试证据，不能替代真实宿主升级旅程。真实第二登录身份、其他计划入口及最终候选回归仍待完成。

目标已移除，原始合成配置字节保持，测试 ACL 已还原。产品操作池的两个文件及私密签名状态按证据/恢复历史保留，未虚报全部删除；见 cleanup-audit.json。未修改日常实例、未提权、未中断系统会话。

原生更新可用 run-native-update.py 和只读 read-update-acl.ps1 复现，固定相同源码/二进制身份，要求新空专用根。此脚本与本次执行逻辑一致，仅公开副本固定了身份并改用同目录权限读取助手。更新和卸载后产品操作池及签名历史保留作证据，不删除未知对象。
