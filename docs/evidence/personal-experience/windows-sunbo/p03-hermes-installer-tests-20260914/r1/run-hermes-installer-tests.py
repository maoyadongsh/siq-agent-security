...
SOURCE = "59d757ae0b104f6d419a7220e64c520727d3c9a7"
BASE = "b303c6f92392f3a44c306d81ad7323c6291ef4f2"
CHANGED = "apps/agentshield/internal/adapterinstall/install_entry_test.go"
...
def main():
    ...
    for name in ("checkout", "go", "git", "pwsh", "private-root", "cache-root", "delivery"):
    ...
    if git("diff", "--name-only", BASE, SOURCE).decode().strip() != CHANGED:
        raise RuntimeError("Expected exactly the reviewed test-file change")
    ...
    env = {
        ...
        "PATH": os.pathsep.join((str(go.parent), str(args.git.resolve().parent), str(system / "System32"))),
    ...
    def run_phase(name, argv, outer_timeout):
        ...
        terminals = [{key: event[key] for key in ("Test", "Action", "Elapsed") if key in event}
    ...
    focused = run_phase("focused", ["test", "-json", "-p=2", "-parallel=1", "-count=1", "-timeout=90s",
                                     "-run", "^(" + "|".join(TESTS) + ")$", "./internal/adapterinstall"], 300)
    phases = [focused]
    if focused["exit_code"] == 0 and not focused["outer_timed_out"]:
        phases.append(run_phase("package", ["test", "-json", "-p=2", "-parallel=1", "-count=1", "-timeout=300s", "./internal/adapterinstall"], 420))
        phases.append(run_phase("vet", ["vet", "-p=2", "./internal/adapterinstall"], 180))
    summary = {
        "candidate": SOURCE, "base": BASE, "source_before": before, "source_after": source_state(),
        "changed_files": [CHANGED], "dco_signoff_present": True, "change_patch_sha256": digest(patch),
        "changed_file_sha256": digest((checkout / CHANGED).read_bytes()),
        ...
        "scope": "Native Windows Go component tests; no real host/model/service acceptance; loopback endpoint strings are configuration fixtures only",
        "phases": phases,
    }
