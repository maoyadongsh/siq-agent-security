...
- 冻结时间（UTC）：2026-09-17T11:59:28Z（本批新候选，测量前）
- 候选：`siq-v6-f04-post-e11`（本地 dirty 集成候选，非发行制品）
- 候选二进制 SHA256：`0b16e5e09e6d807b5be732e390c373e6a4354d096b272eba827bddb84a6c8177`
- 源绑定（本地 dirty 差异清单，非干净发行源）：`ba8d266ef5ba861282f23a8576be8744d6023118f3f42efd8e1054632ffe06fc`
...
- 专属沙箱：`siq-v6-f04-post-e11-local-20260917`（本批所有）
- 证据目录：`docs/evidence/personal-experience/v6-f05-post-e11-perf-r2-20260917/`
...
| 场景 | 旧候选基线 | 理由 |
| --- | --- | --- |
| S1 `doctor_readback` | **not_measured** | 基线为本集成树基点 `9b8c09af…`；此轮只对 post-E11 候选进行绝对预算测量，未按相同协议独占重测基点。既有 GLM d06 二进制不是基点，不能冒充它。 |
...
| R4 | 2026-09-17T11:59:28Z | 本次测量前绑定 post-E11 二进制、源摘要及专属沙箱；保留全部预算、样本、轮次、次序及失败规则。前一个输出目录因缺失协议在零样本处失败，保留为无效预检。 | **否**（本次尚无样本） |
...
该值由驱动常量 `PROTOCOL_SHA256` 绑定，并与同目录 `SHA256SUMS` 一致：
驱动在运行前重算本文件 sha256，与常量不符即**拒绝运行**，因此测后改动预算会被机械拒绝。
