...
TAIL_SCENARIOS = ["C", "D"]  # B0 port: E dropped, RollbackAuthorized absent at b303c6f
...
BUDGET_BASIS = (
    ...
    "component magnitudes; B1-B0 relative budget is P95 increase <=10%. "
    "B0 port note: these budgets are the B1 budgets kept byte-identical; at B0 "
    "the D miss (unbounded fail-return) is the expected finding, not a tuning failure"
)
EVIDENCE_LABEL = {
...
NOT_MEASURED = [
    ...
    {"item": "scenario_E_apply_rollback_cycle", "budget": "p95<=100ms (B1 only)",
     "reason": "B0 (b303c6f) 没有 RollbackAuthorized（O01-O03 新增能力）；无对照路径，B0 侧 not_measured"},
]
# ----------------------------------------------------------------------------
...
SCENARIO_TIMEOUT = {"D": 2700}  # B0 port: D blocks ~30s/probe on inherited pipe (sleep 30)
...
def run_scenario(binary: Path, scenario: str) -> dict:
    ...
    proc = subprocess.run(
        ...
        timeout=SCENARIO_TIMEOUT.get(scenario, 300), capture_output=True, text=True,
...
def main() -> None:
    ...
    report = {
        ...
        "notes": [
            ...
            "B0 port: no pipe drainage exists at b303c6f; D failure return blocks on inherited pipe handles until the hung child exits. The p95<=300ms budget is a B1 budget; the expected B0 miss IS the finding.",
        ],
    }
...
def build_protocol_md() -> str:
    lines = [
        ...
        "",
        f"- 轮数：{ROUNDS}；A/B 交错顺序：{ROUND_ORDERS}；C/D 每轮各测一次（B0 端口：E 无对照路径，见 not_measured）",
