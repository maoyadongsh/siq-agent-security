...
周期计划入口已进入 `0.4.0` 所绑定的源码提交，但个人客户端 Release 不包含企业 Edge/Connector 二进制，不能把个人安装包当作企业设备包。隔离测试证明零/单/多待办、完整性失败、取消与恢复边界；企业 Edge 仍需通过独立候选、签发和部署流程，并补充真实组织身份、systemd 用户服务、长期轮询、升级恢复及多架构原生安装验收。
