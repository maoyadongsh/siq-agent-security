// Package canon produces the canonical JSON byte forms shared with the Python
// control plane.
//
// Marshal  → task signatures: json.dumps(..., ensure_ascii=True)
// MarshalUTF8 → evidence/batch: evidence_signing.canonical_json (ensure_ascii=False)
//
// This is a deliberate copy of apps/agentshield/internal/canon for the Edge
// module (separate go.mod), plus UTF-8 mode for DEV09 evidence. Keep ASCII
// mode in sync with agentshield; UTF-8 mode is Edge/control-plane evidence only.
package canon
...
// map[string]any.
//
// String escaping matches CPython ensure_ascii=True (non-ASCII → \uXXXX).
// Prefer MarshalUTF8 for evidence/batch contracts (ensure_ascii=False).
func Marshal(v any) ([]byte, error) {
	return marshal(v, true)
}

// MarshalUTF8 matches apps/control-api/app/evidence_signing.py canonical_json:
// sort_keys + compact separators + ensure_ascii=False (UTF-8 code points in
// strings; still escapes controls and quotes). Numbers use the same CPython
// float.__repr__ rules as Marshal.
func MarshalUTF8(v any) ([]byte, error) {
	return marshal(v, false)
}

func marshal(v any, escapeASCII bool) ([]byte, error) {
	var buf bytes.Buffer
	if err := write(&buf, v, escapeASCII); err != nil {
		return nil, err
	}
...
func write(buf *bytes.Buffer, v any, escapeASCII bool) error {
	switch t := v.(type) {
	case nil:
	...
	case []any:
		...
		for i, e := range t {
			...
			if err := write(buf, e, escapeASCII); err != nil {
				return err
			}
		}
		buf.WriteByte(']')
	...
	case map[string]any:
		...
		for i, k := range keys {
			...
			writeString(buf, k, escapeASCII)
			buf.WriteByte(':')
			if err := write(buf, t[k], escapeASCII); err != nil {
				return err
			}
		}
		buf.WriteByte('}')
...
// writeString matches CPython json.dumps string rules.
// escapeASCII=true → ensure_ascii=True; false → ensure_ascii=False.
func writeString(buf *bytes.Buffer, s string, escapeASCII bool) {
	buf.WriteByte('"')
	for i := 0; i < len(s); {
		...
		case r == utf8.RuneError && size == 1:
			fmt.Fprintf(buf, `\u%04x`, utf8.RuneError)
		case escapeASCII && r == 0x7f:
			fmt.Fprintf(buf, `\u%04x`, r)
		case escapeASCII && r > 0xffff:
			hi, lo := utf16.EncodeRune(r)
			fmt.Fprintf(buf, `\u%04x\u%04x`, hi, lo)
		case escapeASCII && r >= 0x7f:
			fmt.Fprintf(buf, `\u%04x`, r)
		case r > 0xffff:
			// ensure_ascii=False still uses UTF-8 for supplementary planes.
			buf.WriteRune(r)
		default:
			buf.WriteRune(r)
		}
