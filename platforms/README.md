...
0.3.1 是当前公开签名 Release；平台支持等级仍由实际证据限定。四目标已构建并验签，仅 Linux ARM64 完成本版实际安装链路；同版本的完整升级/回滚与所有宿主旅程尚未验收。`0.4.0-rc.2` 固定在较早源码，当前 `main` 的后续合并尚无新签名候选；已合入共享核心修改需要各平台按同一新候选复测，不能继承其他 OS 或旧候选的通过结果。
