...
func (s *Store) Bind(b Binding) (Binding, error) {
	...
	if existing, err := s.GetBinding(b.BindingID); err == nil {
		if existing.IntentID == b.IntentID && existing.IntentDigest == b.IntentDigest && existing.ExpiresAt == b.ExpiresAt {
			return existing, nil
		}
	...
	if err = publish(p, raw); errors.Is(err, os.ErrExist) {
		...
		if e == nil && existing.IntentID == b.IntentID && existing.IntentDigest == b.IntentDigest && existing.ExpiresAt == b.ExpiresAt {
			return existing, nil
		}
		return b, violation("intent_binding_conflict")
	}
