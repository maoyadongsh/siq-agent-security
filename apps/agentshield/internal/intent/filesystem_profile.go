...
func (c Contract) ResourceProfile() runtimeaction.FilesystemProfile {
	if c.SchemaVersion == "intent/v4" && c.AuthorityKind == "instance_permission" && c.FilesystemProfile == string(runtimeaction.FilesystemWindowsLocalDriveV1) {
...
func (s *Store) checkProfileState(c Contract) error {
	if c.SchemaVersion == "intent/v4" && stateformat.RequireWindowsProfile(filepath.Dir(s.dir)) != nil {
