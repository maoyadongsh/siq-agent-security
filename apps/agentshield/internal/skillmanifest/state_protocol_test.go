...
func TestStateProtocolReleaseContract(t *testing.T) {
	...
	m, e := Build(Options{Matrix: historicalMatrix(t, "skill-manifest.v3.reader3.sample.json"), ClientCompatible: true, Version: "0.3.0-state-protocol", ContentHash: strings.Repeat("a", 64), Artifacts: []Artifact{{OS: "linux", Arch: "arm64", SHA256: strings.Repeat("b", 64), Bytes: 123, URL: "https://example.invalid/siq"}}, SignedBy: key.PublicBase64()})
	if e != nil {
		t.Fatal(e)
	}
