	...
	for _, mode := range []string{"block", "warn", "audit_only"} {
		for _, terminal := range []string{"revoke", "expiry"} {
			t.Run(mode+"/"+terminal, func(t *testing.T) {
				...
				fx.eng.opts.Now = func() time.Time {
					if expired.Load() {
						return issuedAt.Add(time.Minute)
					}
					return issuedAt
				}
				terminalCode := "provenance_issuer_untrusted"
				if terminal == "expiry" {
					terminalCode = "provenance_expired"
				}
				...
				if terminal == "expiry" {
					assertionExpires = issuedAt.Add(time.Minute).Format(time.RFC3339)
				}
				...
				for i := 0; i < 16; i++ {
					...
					go func(i int) {
						...
						a := provenance.Assertion{SchemaVersion: "provenance-assertion/v1", ProvenanceID: id, Source: provenance.Source{Type: "USER", SourceID: "form-field", Trust: "authoritative"}, Scope: scope, ContentDigest: digest, Parents: []string{}, Derivation: "direct", IssuedAt: issuedAt.Add(-time.Minute).Format(time.RFC3339), ExpiresAt: assertionExpires, Issuer: "concurrent-issuer"}
						...
						for j := 0; j < 4; j++ {
							d, err = fx.eng.Decide(own)
							if err != nil || d == nil || (d.Action != ActionAllow && (d.Action != ActionDeny || d.Receipt.AuthorityReasonCode != terminalCode)) {
						...
						for j := 0; j < 4; j++ {
							d, err = fx.eng.Decide(own)
							if err != nil || d == nil || d.Action != ActionDeny || d.Receipt.AdvisoryAction != nil || d.Receipt.AuthorityStatus != "invalid" || d.Receipt.AuthorityReasonCode != terminalCode {
