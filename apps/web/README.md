...
企业导航逐项消费 `console-context` 权限，位于高级页面时自动展开分组但不改 URL；旧 URL、刷新、前进/后退保持可用。资产页包含资产清单、技能清单和框架实例视图；权限、风险、策略与运行时绑定页面提供只作用于已加载记录的筛选和证据详情；环境页可只读分页并管理周期计划；审计页支持精确查询和关联查询。页面隐藏不是授权，后端仍独立校验每个请求；加载失败、未知值和分页未覆盖不能显示成空、零或已安全。
