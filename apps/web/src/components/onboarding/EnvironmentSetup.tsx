...
import { edgeCommands, onboardingApi, type OnboardingAccess, type OnboardingStatus } from '@/api/onboarding';
...
export default function EnvironmentSetup({ environment, access }: { environment: Environment; access: OnboardingAccess }) {
  ...
  useEffect(() => {
    let active = true;
    onboardingApi.status(environment.id).then(value => { if (active) { setProgress(value); setReadError(''); } })
      .catch(() => { if (active) { setProgress(undefined); setReadError('当前无法读取接入进度，请刷新重试。'); } });
    return () => { active = false; };
  ...
  const refresh = () => { setProgress(undefined); setReadError(''); setRetry(n => n + 1); };
  const run = async (kind: 'enroll' | 'scan') => {
    ...
    try {
      if (kind === 'enroll') {
        ...
        setProgress(current);
        if (!current.devices.some(device => device.status === 'online' && device.connectors.includes(connector))) {
          setError('当前没有心跳正常且支持所选采集器的设备，请恢复心跳后再提交。');
          return;
        }
        const task = await onboardingApi.scan(environment.id, connector);
        if (alive.current) { setMessage(`发现任务已提交，请在设备上运行“领取发现任务”命令。任务：${task.task_id}`); refresh(); }
      }
    } catch (e) {
  ...
  const online = progress?.devices.filter(device => device.status === 'online') ?? [];
  const canScan = !!progress && online.some(device => device.connectors.includes(connector));
  return <section className="card" aria-label="环境接入引导">
    <div className="toolbar"><h2>{environment.name} · 接入进度</h2><button className="btn" onClick={refresh}>刷新接入进度</button></div>
    ...
    <h3>3. 发现智能体配置</h3>
    <div className="field"><label htmlFor="edge-connector">发现框架</label><select id="edge-connector" value={connector} onChange={event => setConnector(event.target.value as typeof connector)}><option value="hermes">Hermes</option><option value="openclaw">OpenClaw</option></select></div>
    <p>本次范围：{connector === 'hermes' ? '~/.hermes/profiles/* 下的 config.yaml、SOUL.md' : '~/.openclaw'}。任务由本环境设备领取，多台设备时不保证指定设备执行。</p>
    ...
    {!canScan ? <p>需先收到声明支持所选采集器的设备心跳；请启动心跳后刷新进度。</p> : null}
    ...
    {access.can_view_assets ? <Link to="/agents?view=candidates">查看组织资产清单</Link> : <p>查看资产需要资产读取权限，可联系组织管理员申请。</p>}
  </section>;
}
