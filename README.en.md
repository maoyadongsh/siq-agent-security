...
The **personal client** provides a local service and browser console for agents, Skills and tasks. The **enterprise control plane**, Edge and Connectors support environment inventory, policy approval and deployment readback. The signed `0.3.1` stable release remains available; `0.4.0-rc.2` is a local signed candidate from an earlier main snapshot, while later enterprise-governance, periodic-discovery and audit-export changes in current main have no new signed package. Real business-identity acceptance and convenient LAN team-device workflows remain incomplete.
...
> **Newest signed candidate (2026-09-24):** `0.4.0-rc.2`, built from main `7b68c14`, has been signed with the original publisher key and includes the personal-client/report-tool changes present at that source snapshot. Four-target builds, official-root verification, Linux ARM64 final-package startup and 3/3 tamper rejections passed. It is prepared locally, not publicly released or Latest, and does not include later main changes; upgrade/rollback and full cross-platform acceptance remain pending. Find the offline bundle under `.tmp/releases/0.4.0-rc.2-signed/`; see the [candidate record](docs/evidence/releases/0.4.0-rc.2/README.md). Use its bundled instructions and a separate state directory, not Skill-only network download before publication.
...
**Repository snapshot, 2026-09-27:** Reviewed parallel development has been integrated and pushed to `main`, whose current CI is green. Scope and evidence boundaries are recorded in the [main integration](docs/development/main-branch-integration-20260926.md), [comprehensive acceptance review](docs/development/enterprise-comprehensive-acceptance-review-20260926.md), and [post-merge closeout](docs/development/enterprise-post-merge-remaining-closeout-handoff-20260926.md). **Latest stable remains `0.3.1`, built from `f3d9c3f`; merging source and passing CI do not update an existing release.** The local `0.4.0-rc.2` candidate remains bound to the earlier `7b68c14` source and does not represent later main. Current main still needs source freeze, signing, same-candidate cross-platform installation/upgrade/rollback, and production acceptance.
