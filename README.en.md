...
The **personal client** provides a local service and browser console for agents, Skills and tasks. The **enterprise control plane**, Edge and Connectors support environment inventory, policy approval and deployment readback. Personal client `0.4.0` is signed from the frozen current-main source and published as Latest. Enterprise governance, periodic discovery and audit export are present in the same source commit but are not shipped in the personal-client bundle; enterprise services retain their separate deployment process. Real business-identity acceptance and convenient LAN team-device workflows remain incomplete.
...
> **Signed installation package (2026-09-27):** [0.4.0 release (Latest)](https://github.com/maoyadongsh/siq-agent-security/releases/tag/siq-agent-security-v0.4.0) provides an offline bundle, signed Skill and four target binaries from source `2cd6116`, signed with the existing publisher key. Linux ARM64 passed final-package fresh-state startup and 3/3 tamper rejections. The other targets have build and signed-pin verification only, not native installation, upgrade/rollback, Apple notarization or Authenticode acceptance. Download the eight Release assets; GitHub automatic source archives and the repository Skill remain development source. See [installation instructions](docs/signed-release-packaging.md) and [0.4.0 signing/publication evidence](docs/evidence/releases/0.4.0/README.md).

> **Upgrade boundary:** `0.4.0` includes agent-assisted browser connection, fixed 24-hour management sessions, personal permission facts and batch revocation. Connection requests expire after 5 minutes, and confirming a browser management session does not grant any agent business permission. The stable label does not broaden platform evidence: full cross-platform service installation, upgrade/rollback and host journeys remain open.
...
The product follows **discovery and review → admission and authorization → execution and approval → traceability and maintenance**. Main now includes environment/project discovery, model connection tests, multiple OpenShell gateway checks, read-only grant presets, native protection checks, activity filtering and authorized task-output viewing. The enterprise UI organizes Assets, Permissions, Security and Audit as four primary entries while retaining policy, change, runtime-binding, environment and other advanced management. Framework/role/Skill observations, permission facts, risk handling, exact audit queries and periodic-plan management are integrated without turning observations into effective authority. Priorities are source freeze and same-candidate native acceptance, plus enterprise real-identity/cross-system validation before team-device delivery.
...
**Repository snapshot, 2026-09-27:** Reviewed parallel development is integrated and pushed to `main`; all six GitHub workflows for fixed commit `2cd6116` succeeded, and `0.4.0` is now the signed stable release. Scope and evidence boundaries are recorded in the [main integration](docs/development/main-branch-integration-20260926.md), [comprehensive acceptance review](docs/development/enterprise-comprehensive-acceptance-review-20260926.md), [post-merge closeout](docs/development/enterprise-post-merge-remaining-closeout-handoff-20260926.md), and [release record](docs/evidence/releases/0.4.0/README.md). **Latest is `0.4.0`; a signed personal client does not mean enterprise components are deployed or that unrun native platform acceptance has passed.** Historical host evidence remains bound to its recorded candidate.

| Scope | Current evidence | Remaining work |
| --- | --- | --- |
| Linux personal management | Discovery, model/gateway checks, guided authorization, native protection checks and task outputs are merged; the 0.4.0 package passed Linux ARM64 signature, fresh-state startup and 3/3 tamper rejection | System-service installation, cross-version upgrade/rollback and full host journeys; stock OpenClaw checkpoints and desktop visual checks retain limitations |
| macOS | LaunchAgent lifecycle, staged OpenClaw/Hermes/WorkBuddy work and independent review fixes are merged; 0.4.0 includes a manifest-signed arm64 binary | 0.4.0 native installation/upgrade, same-version host retests, full platform acceptance and Apple signing/notarization; the project manifest signature is not notarization |
| Windows | Writer, migration recovery, DACL, resource/identity checks, host management, task install/upgrade/rollback and junction protection are merged; 0.4.0 includes a manifest-signed amd64 binary and Skill | 0.4.0 native installation/upgrade and same-version host retests; historical OpenClaw evidence uses a WSL Agent, Hermes uses native CLI, and minimal WorkBuddy native I/O does not establish full approval recovery, desktop update or repeatability |
...
| LAN team management | Enterprise API / Web, periodic-plan storage/management, explicit device confirmation/polling/retirement and read-only pending-plan discovery are merged; Control API, Edge and Connectors provide centralized governance | Real organization accounts, independent approvers and cross-system result access; real-device continuous collection, installation handoff and recovery; convenient team-device enrollment/management acceptance |
...
**Local deployment, September 25:** The historical deployment record describes `0.4.0-dev-console` redirecting to `/agents` while retaining Skill-assisted connection, 24-hour sessions, identity and history. Its “not publicly released” statement was true at that snapshot; these personal features are now in the 0.4.0 stable package. See the [deployment and rollback record](docs/development/personal-console-deployment-20260925.md).

The same day's **console simplification source increment** adds four primary workflows—My Agents, Permissions, Security, and Runtime Audit—plus evidence-based framework/role/Skill navigation and batch revocation with signed previews, per-item revision checks, and audit records. Existing defenses and advanced workspaces remain; bulk permission expansion is not enabled. These capabilities are now in the 0.4.0 stable package. See the original [scope and validation](docs/development/personal-console-simplification-20260925.md).

**Personal browser connection:** 0.4.0 offers “Connect through your agent.” Send the page's public connection request to the installed SIQ Skill; explicit local confirmation connects the originating browser without copying a pairing code. Requests expire after 5 minutes. Management sessions last a fixed **24 hours**; reload does not extend expiry, and logout or service restart requires reconnection. Manual pairing remains available. Connecting the browser does not grant agent permissions. See the [operation guide](docs/personal-client-operation-guide-20260916.md).
...
The [September 24 enterprise deployment](docs/development/enterprise-runtime-delivery-20260924.md) verified the then-current `0013→0017` migration, isolated backup restoration, image switching and desktop/mobile Gateway login entry points. Current main has migrations through `0028`; deployments must always run `alembic upgrade head`, not stop at the historical migration number. Real-account login and cross-system authorization journeys remain incomplete. Periodic-plan storage, confirmation, management, device polling/retirement and read-only pending-plan discovery are now merged (see the [auto-onboarding closeout](docs/development/enterprise-auto-onboarding-closeout-20260926.md) and [post-merge handoff](docs/development/enterprise-post-merge-remaining-closeout-handoff-20260926.md)); discovery only reports that a plan awaits confirmation, and current evidence does not establish complete real-device continuous scanning. `X-SIQ-Export-Truncated` reports response truncation but does not make OCSF export a complete archive or retention/deletion policy. See the [two-step example](#enterprise-edge-new-device-periodic-discovery-two-steps), [OCSF contract](packages/contracts/enterprise-ocsf-export.v1.md), and [production runbook](docs/enterprise-production-runbook-v1.md). Historical “uncommitted/unpublished/undeployed” notes describe their original snapshots; use later integration and Git records for current status.
...
For the signed version, download the [0.4.0 offline bundle](https://github.com/maoyadongsh/siq-agent-security/releases/download/siq-agent-security-v0.4.0/siq-agent-security-0.4.0-bundle.zip) and follow the [installation instructions](docs/signed-release-packaging.md) to extract it, verify the package and start the local service. The bundled `INSTALL.md` includes first-start instructions: verify the package, then use the verified binary’s `start` command to initialize state and start the service. Direct bootstrap use still requires initialization beforehand. No local Go/UI compilation is needed. The source-development and research steps remain below; GitHub automatic Source code archives are not signed installation packages.
...
Updated through **2026-09-27**. Recent engineering/release checks and historical research reproduction are listed separately. Each result belongs to its recorded source or candidate, environment and test scope; denominators must not be pooled or transferred to other versions.

**Recent engineering and signed-release verification (2026-09-18–27)**

| Archived observation | Result | Evidence and scope |
| --- | --- | --- |
| 0.4.0 stable release | Official signature, four pins, Linux ARM64 final-package startup and **3/3** tamper rejection passed; remote **8/8** assets match byte for byte | [Release record](docs/evidence/releases/0.4.0/README.md); fixed `2cd6116`, regular release and Latest; no other-target native installation or cross-platform upgrade/rollback acceptance |
| Current main integration and acceptance | Post-merge Control API **2257 passed / 1 skipped**; the comprehensive review separately recorded Web **1012 passed**, 14 Go modules and **30/30** isolated browser journeys; current main CI is green | [Main integration](docs/development/main-branch-integration-20260926.md), [comprehensive review](docs/development/enterprise-comprehensive-acceptance-review-20260926.md), [post-merge handoff](docs/development/enterprise-post-merge-remaining-closeout-handoff-20260926.md); separate runs and counting units, not release or production acceptance |
...
| 0.3.1 publication and source CI | **8/8** assets match; **6/6** source workflows succeeded | [Public readback](docs/evidence/releases/0.3.1/publication.json), [source CI](docs/evidence/releases/0.3.1/source-ci.json); regular release and Latest at the time of readback; workflow count is not a test count |
...
| Edge and Connectors | Configuration, directory, framework, process, container and cluster collection; SIQ reads local business-security event projections without business credentials or database access. Periodic plan storage, explicit device confirmation/polling/retirement and read-only pending-plan discovery are implemented, but real-device continuous collection and production onboarding remain acceptance gates | [Edge](edge/agent/README.md), [Connectors](connectors/README.md), [SIQ Connector](connectors/siq/README.md), [compatibility](docs/compatibility.md) |
...
1. **Continue 0.4.0 platform acceptance.** The stable release has a fixed source inventory, original publisher signature and completed public readback. Complete native installation on the other three targets plus same-version system-service, upgrade, rollback and exit checks across all four targets; fill platform signing/notarization and compatibility gaps.
...
Use **[CITATION.cff](CITATION.cff)** for the project title and author information; its version/date currently identify `research-v0.1.0-rc.1`. When citing personal-client 0.4.0, specify that release tag, source `2cd6116` and the actual artifact digest; experiments also need their corpus digest. See the [citation guide](docs/research/citation-guide.md).
