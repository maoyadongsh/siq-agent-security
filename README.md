...
控制台需要 `serve` 终端里打印的一次性配对码，**不要**把配对码或 `token` 文件贴进聊天或截图。适配器仍读状态目录里的决策 token；该 token **不能**调用批准/卸载等管理接口。当前桌面模式是同一 OS 用户，**不**宣称能阻止被注入的 Agent 自己执行 CLI。
