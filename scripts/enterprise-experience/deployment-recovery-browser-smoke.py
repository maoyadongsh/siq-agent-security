...
def main():
    ...
    checks = {}
    with tempfile.TemporaryDirectory(prefix='siq-recovery-e150-') as raw:
        root = Path(raw)
        ...
        with (root / 'server.log').open('w') as log:
            ...
            try:
                ...
                changes = {}
                for key in ('normal', 'lost', 'empty', 'cancel', 'stale', 'reload'):
                ...
                with sync_playwright() as pw:
                    ...
                    # Preview can be cancelled without creating any deployment.
                    page.locator('tr').filter(has_text=changes['cancel']['id']).get_by_role('button', name='部署', exact=True).click()
                    preview_dialog = page.get_by_role('dialog', name='确认部署目标')
                    expect(preview_dialog.get_by_text('fixture-research-sandbox', exact=True)).to_be_visible()
                    expect(preview_dialog.get_by_text('开发测试后端', exact=True)).to_be_visible()
                    page.screenshot(path=str(args.out_dir / 'preview-desktop.png'), full_page=True, animations='disabled')
                    page.set_viewport_size({'width': 390, 'height': 844})
                    assert preview_dialog.evaluate('(el) => el.scrollWidth <= el.clientWidth + 1')
                    cancel_box = preview_dialog.get_by_role('button', name='取消', exact=True).bounding_box()
                    assert cancel_box and cancel_box['y'] >= 0 and cancel_box['y'] + cancel_box['height'] <= 844
                    page.screenshot(path=str(args.out_dir / 'preview-mobile.png'), full_page=True, animations='disabled')
                    page.set_viewport_size({'width': 1440, 'height': 1000})
                    checks['preview_mobile_actions_visible'] = True
                    preview_dialog.get_by_role('button', name='取消', exact=True).click()
                    assert api('/api/v1/change-requests/' + changes['cancel']['id'] + '/execution')['deployments'] == []
                    checks['preview_cancel_has_no_deployment'] = True
                    # A concurrent deployment changes state after this browser preview.
                    page.locator('tr').filter(has_text=changes['stale']['id']).get_by_role('button', name='部署', exact=True).click()
                    preview_dialog = page.get_by_role('dialog', name='确认部署目标')
                    expect(preview_dialog.get_by_text('fixture-research-sandbox', exact=True)).to_be_visible()
                    stale_body = {'schema_version': 'deployment-preview-request/v1', 'change_request_id': changes['stale']['id'], 'environment_id': 'env-execution', 'binding_id': binding['id']}
                    old_preview = api('/api/v1/deployment-preview', stale_body)
                    # Separate approved deployment changes the CR state, as another operator could do.
                    api('/api/v1/deployment-preview/submit', {**stale_body, 'schema_version': 'deployment-preview-submit/v1', 'preview_digest': old_preview['preview_digest']}, expected=201)
                    preview_dialog.get_by_role('checkbox').check()
                    preview_dialog.get_by_role('button', name='确认并部署').click()
                    expect(preview_dialog.get_by_text('此变更当前不可部署', exact=False)).to_be_visible()
                    expect(preview_dialog.get_by_role('button', name='确认并部署')).to_have_count(0)
                    assert len(api('/api/v1/change-requests/' + changes['stale']['id'] + '/execution')['deployments']) == 1
                    preview_dialog.get_by_role('button', name='取消', exact=True).click()
                    checks['changed_state_requires_new_preview_without_duplicate_deployment'] = True
                    ...
                    preview_dialog.get_by_role('checkbox').check()
                    preview_dialog.get_by_role('button', name='确认并部署').click()
                    expect(dialog.get_by_text('已找回本次部署请求', exact=False)).to_be_visible()
                    expect(dialog.get_by_text('已创建下发任务', exact=False)).to_be_visible()
                    ...
                    page.reload()
                    expect(dialog.get_by_text('已找回本次部署请求', exact=False)).to_be_visible()
                    checks['persisted_request_recovered_after_reload'] = True
                    ...
                    page.evaluate("""() => {
                        ...
                        window.fetch = async (...args) => {
                            ...
                            if (String(args[0]).endsWith('/deployment-submissions') && response.status === 201) {
                                window.submissionCommitted = true;
                                return new Promise(() => {});
                            }
                            return response;
                        };
                    }""")
                    page.locator('tr').filter(has_text=changes['reload']['id']).get_by_role('button', name='部署', exact=True).click()
                    preview_dialog = page.get_by_role('dialog', name='确认部署目标')
                    preview_dialog.get_by_role('checkbox').check()
                    preview_dialog.get_by_role('button', name='确认并部署').click()
                    page.wait_for_function('window.submissionCommitted === true')
                    assert 'view=execution' in page.url and changes['reload']['id'] in page.url
                    expect(preview_dialog.get_by_role('button', name='正在提交…')).to_be_disabled()
                    page.reload()
                    expect(dialog.get_by_text('已找回本次部署请求', exact=False)).to_be_visible()
                    expect(dialog.get_by_text('保存部署请求', exact=True)).to_be_visible()
                    stored = api('/api/v1/change-requests/' + changes['reload']['id'] + '/deployment-submission')
                    actual = api('/api/v1/change-requests/' + changes['reload']['id'] + '/execution')
                    assert len(actual['deployments']) == 1 and actual['deployments'][0]['id'] == stored['deployment_id']
    ...
    (args.out_dir / 'result.json').write_text(json.dumps({'schema_version': 'enterprise-deployment-recovery-smoke/v1',
