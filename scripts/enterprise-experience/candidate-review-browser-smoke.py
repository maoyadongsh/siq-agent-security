        ...
        with tempfile.TemporaryFile() as logs:
            ...
            try:
                ...
                with sync_playwright() as pw:
                    ...
                    # Document overflow can be clipped by CSS while actions remain
                    # unreachable. Check the actual controls, not just scrollWidth.
                    assert page.locator('.page-header-right').evaluate('''el =>
                        [...el.querySelectorAll('button, a')].every(control => {
                            const r = control.getBoundingClientRect();
                            return r.left >= 0 && r.right <= innerWidth;
                        })'''), 'mobile header action is clipped'
                    checks['mobile_header_actions_inside_viewport'] = True
