...
try {
  ...
  for (const entry of spec.cases) {
    ...
    const execution = tool.execute(entry.id, { command: "printf fixture" });
    ...
    while (!fs.existsSync(choicePath)) {
    ...
    try {
      ...
      assert.equal(choice.decision, "deny", "unexpected execution failure after platform approval");
      assert.equal(error.message, "Denied by user", "unexpected native rejection category");
    ...
    const output = { id: entry.id, executed, platform_decision: choice.decision,
