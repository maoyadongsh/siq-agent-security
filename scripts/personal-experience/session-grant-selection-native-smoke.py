#!/usr/bin/env python3
"""Verify fixed Grant selection through the product self-check and public Hermes CLI."""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="siq-session-grant-selection-") as temporary:
        root = Path(temporary)
        ...
        try:
            ...
            fixture.require(len(bindings) == 1, "expected exactly one self-check session")
            selected = bindings[0].get("grant_ref")
            fixture.require(
                selected and selected["grant_id"] == grants[0]["grant_id"]
                and selected["admission_id"] == grants[0]["admission_id"]
                and len(selected["permission_digest"]) == 64,
                "native session did not select its own fixed Grant",
            )
            ...
            report = {
                "schema_version": "session-grant-selection-native-smoke/v1",
                ...
                "checks": {
                    ...
                    "grant_revoked": True,
                    "native_session_selected_own_grant": True,
