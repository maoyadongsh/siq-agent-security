...
Uses managed installation, permission revision drafts and public native enable CLI.
Checks ordinary automatic enrollment, the independent product self-check, and
revocation in an isolated profile. Browser approval is verified separately.
"""
...
class Harness(fixture.Harness):
    ...
    def managed_selfcheck(self):
        plan = self.api(
            "/v1/runtime-checks/preview",
            {
                "schema_version": "local-runtime-check-preview/v1",
                "instance_id": self.instance_id,
            },
        )
        result = self.api(
        ...
        while time.monotonic() < deadline:
            result = self.api("/v1/runtime-checks/" + plan["check_id"])
            if result["status"] in ("passed", "failed", "invalidated", "cancelled"):
                break
            time.sleep(1)
        fixture.require(
            result["status"] == "passed" and result["cleanup"] == "complete",
            "managed profile self-check failed: " + result["reason_code"],
        )
        fixture.require(len(result["receipt_ids"]) == 5, "self-check receipt set incomplete")
        identities = self.api("/v1/runtime-identities")["items"]
        fixture.require(
            next(i for i in identities if i["identity_id"] == self.issued["identity"]["identity_id"])["status"]
            == "issued",
            "temporary self-check revoked ordinary identity",
        )
        bindings = self.api("/v1/intent-bindings")["items"]
        fixture.require(
            len(bindings) == 2 and sum(b["grant_ref"] == self.issued["identity"]["grant_ref"] for b in bindings) == 1,
            "self-check borrowed ordinary permission envelope",
        )
        return {
            "status": result["status"],
            "cleanup": result["cleanup"],
            "receipt_count": 5,
            "ordinary_identity_retained": True,
            "separate_grant_binding": True,
        }
    ...
    def public_cli(self):
        ...
        try:
            ...
            fixture.require(signed_count == 10, "combined receipt set incomplete")
            self.api(
            ...
            fixture.require(
                after_revoke.returncode == 0 and not failures and len(received) == 4,
            ...
            fixture.require(
                len({row["session_id"] for row in pending}) == 1 and pending[0]["session_id"] != session,
                "revoked run did not use a new native session",
            )
            fixture.require(not forbidden.exists(), "revoked write executed")
            revoked_requests = list(received)
            source = self.api("/v1/grants/" + self.issued["identity"]["grant_ref"]["grant_id"])
            drafted = self.api(
                "/v1/grants/" + source["grant"]["grant_id"] + "/draft",
                {
                    "schema_version": "grant-draft-create/v1",
                    "expected_revision": source["state_revision"],
                    "actor_id": "synthetic-revision-operator",
                    "request_id": "gd-" + "a" * 32,
                },
            )
            draft_path = "/v1/grants/" + drafted["grant"]["grant_id"]
            changed = self.api(
                draft_path + "/resources",
                {
                    "schema_version": "grant-resource-edit/v1",
                    "expected_revision": drafted["state_revision"],
                    "actor_id": "synthetic-revision-operator",
                    "tools": ["read_file", "write_file"],
                    "filesystem": {"read_only": [str(self.workspace / "company-a")], "read_write": []},
                    "network": [],
                    "models": [],
                },
            )
            challenge = self.api(
                draft_path + "/challenge",
                {
            ...
            issued = self.api(
                "/v1/runtime-identities",
                {
                    "schema_version": "local-runtime-identity-create/v1",
                    "instance_id": self.instance_id,
                    "grant_id": deployed["grant"]["grant_id"],
            ...
            plan = self.api(
                "/v1/adapter/preview",
                {
                    ...
                    "instance_id": self.instance_id,
                    "native_enable": True,
                    "runtime_identity_id": issued["identity"]["identity_id"],
                },
            )
            self.api(
                "/v1/adapter/install",
            ...
            new_token = Path(issued["credential_path"]).read_text().strip()
            self.api(
                "/v1/runtime-sessions",
            ...
            calls[-1] = {
                "id": "outside-denied",
                "tool": "read_file",
                "params": {"path": str(self.workspace / "company-b/report.txt")},
            }
            ...
            fixture.require(
                replacement.returncode == 0 and not failures and len(received) == 4,
                "replacement native conversation did not enforce new scope",
            )
            appended = self.receipts()[signed_count:]
            promoted = [r for r in appended if r.get("matched_rule_ids") == ["pending.fail_closed"]]
            expected_pending = {
                hashlib.sha256(
                    "|".join(
                        str(row.get(key, ""))
                        for key in ("recorded_at", "platform", "tool", "session_id", "outcome", "reason")
                    ).encode()
                ).hexdigest()
                for row in pending
            }
            fixture.require(
                ...
                and len({r["params_digest"] for r in promoted}) == len(promoted)
                and all(
                    r["params_digest"] in expected_pending
                    and r["action"] == "deny"
                    and r["session_id"] == pending[0]["session_id"]
                    for r in promoted
                ),
                "unexplained or duplicate promoted pending record",
            )
            new_records = [r for r in appended if r not in promoted]
            fixture.require(
                len(new_records) == 4
                and len({r["session_id"] for r in new_records}) == 1
                and new_records[0]["session_id"] != session,
                "replacement session evidence incomplete",
            )
            native_sid = new_records[0]["session_id"]
            bindings = self.api("/v1/intent-bindings")["items"]
            bound = next(b for b in bindings if b["session_id"] == native_sid)
            fixture.require(
                bound["grant_ref"] == issued["identity"]["grant_ref"], "replacement selected old permission"
            )
            decisions = {r["tool_call_id"]: r for r in new_records if r["record_type"] == "decision"}
            fixture.require(
                decisions["allowed-first"]["action"] == "allow"
                and decisions["write-denied"]["action"] == "deny"
                and decisions["outside-denied"]["action"] == "deny"
                and decisions["outside-denied"]["reason_code"] == "grant_scope_violation",
                "new scope did not reject outside read at Grant boundary",
            )
            fixture.require(
                all(r["matched_grant_id"] == issued["identity"]["grant_ref"]["grant_id"] for r in decisions.values()),
                "mixed Grant references in native decisions",
            )
            fixture.require(
                self.api("/v1/grants/" + source["grant"]["grant_id"]) == source, "replacement modified historical Grant"
            )
            fixture.require(not forbidden.exists(), "replacement executed forbidden write")
            signed_count += len(appended)
            self.stop()
            verified = json.loads(self.command([str(self.binary), "verify"]))
            fixture.require(verified["verified"], "receipt chain invalid")
            return {
                "schema_version": "personal-permission-revision-native-smoke/v1",
                ...
                "checks": [
                    ...
                    "native_enable_preserves_other_profile_and_settings",
                    "diagnosis_keeps_runtime_unverified",
                    "managed_profile_product_selfcheck_uses_separate_authority",
                    "new_permission_draft_has_independent_namespace",
                    "replacement_identity_cannot_take_over_old_session",
                    "new_native_session_uses_new_fixed_grant",
                    "new_native_scope_rejects_previous_allowed_directory",
                    "historical_grant_preserved_after_replacement",
                    "native_cli_started",
                ...
                "revoked_model_requests": revoked_requests,
                "replacement_model_requests": received,
                "replacement_receipt_count": len(new_records),
                "promoted_pending_denials": len(promoted),
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="siq-permission-revision-native-") as temporary:
        root = Path(temporary)
